...
// 주어진 경로에서 파일과 디렉터리 목록을 재귀적으로 출력하는 함수
// depth는 현재 디렉터리의 깊이를 나타내어 들여쓰기에 사용된다.
fn list_files_and_directories(path: &std::path::Path, depth: usize) -> io::Result<()> {
    if path.is_dir() {
        ...
        for entry in entries {
            ...
            // 항목의 이름 추출
            let file_name = entry_path.file_name().and_then(|os_str| 
            os_str.to_str()).unwrap_or("");
            println!("{:indent$}{}", "", file_name, indent=depth);

            // 항목이 디렉터리인 경우, 해당 디렉터리 내의 파일 및 서브 디렉터리를 출력하기 위해 재귀적으로 호출
